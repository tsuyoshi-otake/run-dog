...
/// Drive that owns a Windows directory (`D:\Windows\System32` → `D:\`).
///
/// Relative paths are rejected. There is no `C:\` fallback.
#[must_use]
...
mod tests {
    ...
    fn component_volume_root_resolves_non_c_windows_directory() {
        ...
        assert_eq!(volume_root_from_windows_dir(Path::new("")), None);
    }

    #[test]
    fn component_storage_source_does_not_hardcode_c_volume() {
        let production = include_str!("storage.rs")
            .split("#[cfg(test)]")
            .next()
            .expect("production source");
        assert!(production.contains("GetSystemDirectoryW"));
        assert!(
            !production.contains("\"C:\\\\\""),
            "query path must come from GetSystemDirectoryW"
        );
    }
}
