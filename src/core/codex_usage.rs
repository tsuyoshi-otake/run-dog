//! Codex `token_count` continuation: count advancing usage, ignore snapshot replay.
//!
//! Session identity is the cumulative `total_token_usage` watermark. There is no
//! separate epoch id in the JSONL, so a smaller total is treated as a reset
//! rather than delayed out-of-order data. Missing totals are counted without
//! claiming replay protection.
