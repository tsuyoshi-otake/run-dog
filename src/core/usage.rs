...
mod tests {
    ...
    #[test]
    fn component_split_event_cost_matches_one_batch_after_display_rounding() {
        let one = TokenUsage {
            input: 1,
            ..TokenUsage::default()
        };
        ...
        let nanos_sum: u64 = (0..10_000)
            .map(|_| cost_nanos("claude-opus-5", one, None).expect("priced"))
            .sum();
        let batch_nanos = cost_nanos("claude-opus-5", batch, None).expect("priced");

        assert_eq!(cost_cents("claude-opus-5", one, None), Some(0));
    ...
    proptest::proptest! {
        #[test]
        fn pbt_split_event_cost_matches_one_batch(
            tokens in 1u64..8_000u64,
            parts in 1u64..64u64,
        ) {
            let batch = TokenUsage {
                input: tokens,
            ...
            let remainder = tokens % parts;
            let mut split_nanos = 0_u64;
            for index in 0..parts {
                let input = base + u64::from(index < remainder);
                split_nanos = split_nanos.saturating_add(
                    cost_nanos(
                        "claude-opus-5",
                        TokenUsage {
                            input,
            ...
            proptest::prop_assert_eq!(split_nanos, batch_nanos);
            proptest::prop_assert_eq!(display_cents(split_nanos), display_cents(batch_nanos));
        }
    }
