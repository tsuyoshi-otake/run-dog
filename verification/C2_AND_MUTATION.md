...
範囲 `src/core` + `src/application` の branch coverage は **47/50 (94.0%)**、line coverage は **585/600 (97.5%)**。移動前の生成 target に由来するゼロ計測の旧パスは JSON に残るため、現在の root `C:\\path\\to\\run-dog` に限定して集計した。未カバー3 branch は主に Windows 非ライブ境界ではなく、`ports.rs` の loop / pattern と animation の補助分岐である。数値を C2 / MC/DC と読み替えてはいけない。
