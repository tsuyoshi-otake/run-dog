...
coverage JSON には移動前の生成 target に由来するゼロ計測の旧パス `C:\\path\\to\\run-dog` が残る。報告した 585/600 と47/50は現在の root `C:\\path\\to\\run-dog` の `src/core` と `src/application` だけをフィルタした値であり、旧パスを分母に混ぜていない。
